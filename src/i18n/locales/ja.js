...
export default {
  ...
  home: {
    ...
    feature1Text:
      '学校や就寝時間に合わせて、1 日の上限と利用禁止時間を設定できます。時間切れになると端末は自動でロックされます。',
  ...
  support: {
    ...
    start4:
      '**4. 子どもの端末で権限を許可する。** 子どもの端末で *ステータス* 画面を開き、KidGate が求めるすべての権限を許可します。Android では通知、使用状況へのアクセス、他のアプリの上に表示、ユーザー補助、バッテリー無制限。iOS では *App とウェブサイトのアクティビティを許可*（スクリーンタイム）。これらがオンになるまで、コントロールは完全には機能しません。',
    start5:
      '**5. コントロールを設定する。** 保護者の端末で子どもの端末カードを開き、1 日の上限、利用禁止時間、ブロックするアプリ、ウェブフィルタ、位置情報の各機能を設定します。',
    startNote:
    ...
    faq8A:
      'Android では、ロックに *他のアプリの上に表示* と *ユーザー補助* のヘルパーの有効化、さらにバッテリー無制限が必要です。Xiaomi、Samsung、Oppo、Vivo などの端末では、自動起動も許可し、「スリープ中のアプリ」の一覧から KidGate を外してください（子どもの端末の *ステータス → KidGate を動かし続ける* を参照）。iOS ではロックはスクリーンタイムの許可に依存します。あとから権限がオフになった場合は、保護者の端末に保護アラートが届きます。',
