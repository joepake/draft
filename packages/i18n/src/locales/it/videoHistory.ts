export const videoHistory = {
  title: 'Video guardati',
  fallbackDeviceName: 'Dispositivo del bambino',
  toggleLabel: 'Registra i video guardati',
  toggleHint:
    'YouTube nel browser e l’app YouTube su telefoni Android e TV. Sulla TV gli Shorts non vengono elencati perché l’app TV non li identifica come Shorts.',
  toggleFailed: 'Impossibile aggiornare la cronologia video. Riprova.',
  unsupportedNote:
    'Questo dispositivo non può segnalare i video guardati. Funziona su Android, Android TV e in Chrome.',

  unsupportedNeedsExtension:
    'Su questo computer KidGate non può vedere quali video vengono guardati: legge i nomi dei siti, non il contenuto di una pagina. L’estensione KidGate per Chrome sì: aggiungila su questo computer e segnalerà i video guardati come un dispositivo a sé.',
  viaExtension: 'Tramite estensione',
  extensionGuideTitle: 'Configurala in Chrome',
  extensionStepOpenChrome: 'Apri Chrome su questo computer',
  extensionStepStore: 'Vai al Chrome Web Store',
  extensionStepSearch: 'Cerca “KidGate”',
  extensionStepInstall: 'Fai clic su “Aggiungi a Chrome”',
  extensionStepConnect:
    'Apri KidGate in Chrome e collegala: compare come dispositivo a sé',
  extensionStepEnable: 'Attiva «Registra i video guardati» per quel dispositivo',
  emptyTitle: 'Ancora nessun video guardato',
  emptyBody: 'I video appaiono qui quando tuo figlio guarda con KidGate attivo.',
  emptyBodyOff: 'Attiva la registrazione sopra per elencare i video guardati.',
  viewsMeta_one: '{{count}} volta',
  viewsMeta: '{{count}} volte',
  dayToday: 'Oggi',
  dayYesterday: 'Ieri',
  dayCount_one: '{{count}} video',
  dayCount: '{{count}} video',
  heroVideos: 'Video',
  heroChannels: 'Canali',
  heroViews: 'Riproduzioni',
  emptyStepEnable: 'Attiva la registrazione',
  emptyStepWatch: 'Tuo figlio guarda YouTube',
  emptyStepAppear: 'I video compaiono qui',
  unknownChannel: 'Canale sconosciuto',
  heroTopChannel: 'Più guardato',
  readerLayoutChanged:
    'Al momento gli Shorts non vengono registrati su questo dispositivo: l’app YouTube è cambiata e KidGate ha bisogno di un aggiornamento. Gli altri video vengono ancora registrati.',
  openAction: 'Apri su YouTube',
  searchAction: 'Cerca questo video su YouTube',
  openFailed: 'Impossibile aprire YouTube.',
};
