export const videoHistory = {
  title: 'Vídeos assistidos',
  fallbackDeviceName: 'Dispositivo da criança',
  toggleLabel: 'Registrar vídeos assistidos',
  toggleHint:
    'YouTube no navegador e o app do YouTube no Android e na TV. Na TV, os Shorts não aparecem porque o app de TV não os identifica como Shorts.',
  toggleFailed: 'Não foi possível atualizar o histórico de vídeos. Tente novamente.',
  unsupportedNote:
    'Este dispositivo não consegue informar os vídeos assistidos. Funciona no Android, no Android TV e no Chrome.',

  unsupportedNeedsExtension:
    'O KidGate neste computador não consegue ver que vídeos são vistos — lê nomes de sites, não o conteúdo de uma página. A extensão do KidGate para o Chrome consegue: adicione-a neste computador e ela reporta os vídeos vistos como um dispositivo próprio.',
  viaExtension: 'Via extensão',
  extensionGuideTitle: 'Configure no Chrome',
  extensionStepOpenChrome: 'Abra o Chrome neste computador',
  extensionStepStore: 'Vá até a Chrome Web Store',
  extensionStepSearch: 'Pesquise “KidGate”',
  extensionStepInstall: 'Clique em “Usar no Chrome”',
  extensionStepConnect:
    'Abra o KidGate no Chrome e conecte: ele aparece como um dispositivo próprio',
  extensionStepEnable: 'Ative «Registrar vídeos assistidos» nesse dispositivo',
  emptyTitle: 'Nada assistido ainda',
  emptyBody:
    'Os vídeos aparecem aqui quando seu filho assiste com o KidGate em execução.',
  emptyBodyOff: 'Ative o registro acima para começar a listar os vídeos assistidos.',
  viewsMeta_one: '{{count}} vez',
  viewsMeta: '{{count}} vezes',
  dayToday: 'Hoje',
  dayYesterday: 'Ontem',
  dayCount_one: '{{count}} vídeo',
  dayCount: '{{count}} vídeos',
  heroVideos: 'Vídeos',
  heroChannels: 'Canais',
  heroViews: 'Reproduções',
  emptyStepEnable: 'Ative o registo',
  emptyStepWatch: 'O seu filho vê YouTube',
  emptyStepAppear: 'Os vídeos aparecem aqui',
  unknownChannel: 'Canal desconhecido',
  heroTopChannel: 'Mais visto',
  readerLayoutChanged:
    'No momento, os Shorts não estão sendo registrados neste dispositivo: o app do YouTube mudou e o KidGate precisa de uma atualização. Os outros vídeos continuam sendo registrados.',
  openAction: 'Abrir no YouTube',
  searchAction: 'Pesquisar este vídeo no YouTube',
  openFailed: 'Não foi possível abrir o YouTube.',
};
