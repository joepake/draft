...
export const report = {
  ...
  /*
   * The week in progress, and when it will be reported on.
   *
   * A weekday and no clock time, deliberately. The push goes out around 08:00
   * on each parent's own phone, but the stored offset is a fixed number rather
   * than a timezone, so a printed "08:00" is an hour wrong in every country
   * that observes daylight saving. "Monday morning" survives that, and it is
   * also what a parent actually wants to know.
   */
  currentWeekTab: 'This week',
  currentWeekTitle: 'Still being measured',
  currentWeekBody: 'The report for this week arrives {{day}}.',
  currentWeekNote:
    'It covers Monday to Sunday, across every device in the family. Nothing is missing yet — the week has not finished.',
  ...
  // Shown in place of the minutes figure on a child row with no assigned
  // device — tapping the row opens the assign sheet instead of a report.
  hubChildUnassigned: 'No device assigned',
  ...
  guestDescription:
    'Sign in to measure today against a normal day, compare your children side by side, and get a written report every Monday.',
  ...
  guestBenefitWeeklyTitle: 'A report every Monday',
