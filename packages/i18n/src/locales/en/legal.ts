  ...
  privacyPolicy: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: 'Three features use Google’s Gemini models, reached through Google Cloud: the weekly report’s written summary, which is generated from a family’s own usage figures; the classification of apps and website domains into categories the Web Filter and the app lists use, kept in a shared table that records a domain and its category, not which family visited it; and the confirmation step described in section 3, which runs only where a parent has consented to it. These models produce judgements that can be wrong. A category, a summary sentence or a message alert is a prompt to look, not a finding of fact, and KidGate makes no decision with a legal or similarly significant effect on a child on the basis of one. Model output is not used to train Google’s models.',
      },
      {
        title: '6. Legal grounds and consent',
        body: 'KidGate processes data to provide requested services, meet legal duties, protect legitimate safety and security interests, or based on consent where required. Parents are responsible for giving required notices and obtaining valid consent for a child or device user. Message monitoring and AI confirmation are each a separate, explicit opt-in and can be withdrawn at any time: message monitoring is switched on per device, and AI confirmation is agreed once for the family and recorded with who agreed and when.',
      },
      {
        title: '7. Service providers',
        body: 'KidGate is built on Google Cloud and Firebase, which provide authentication, the database, file storage, the server functions, push notifications through Firebase Cloud Messaging, crash reporting through Firebase Crashlytics, and the Gemini models named in section 5. Apple and Google also process subscription purchases, renewals and refunds through their app stores, and Google Analytics processes the measurement described in section 8. Four other providers each see one thing: HERE Technologies turns a reported position into an address and draws the map tiles behind the location screens, so it receives the coordinates a device reported; Resend delivers the emails KidGate sends — an SOS alert to a trusted contact, and the weekly report; Cloudflare serves the desktop installers; and the unpkg content network serves the map library to the parent apps. Data is disclosed to these providers only as needed to run the service; to authorities where legally required; or to address safety, fraud, or abuse. Providers have their own duties and policies, and none is authorized by KidGate to use child data for independent marketing.',
      },
      {
        title: '8. Website analytics and cookies',
        body: 'The kidgate.app website measures three things with Google Analytics: page visits, and clicks on each of the two desktop download links. Analytics storage is declined by default, so no analytics cookie is set and nothing is stored in the reader’s browser; each visit is counted from a cookieless request with the IP address truncated. Google Signals and advertising identifiers are off, and nothing identifying a reader or a family is sent. The KidGate apps report a small set of events to the same property to show which features are used; those events carry an app-instance identifier — never the device’s advertising identifier, which the apps do not collect — and never a child’s name, message, location or browsing. The apps and the website use no advertising or tracking network of any kind.',
      },
      {
      ...
      },
      {
        ...
        body: 'Where it is necessary to answer a support request or diagnose a fault, authorised KidGate personnel can open a family account and see what a parent sees: its configuration and devices, and the activity within it — including location history, web history, message alerts and photos attached to an SOS or a Check-In. They can also change settings and issue device commands. Such access is limited to authorised personnel, requires two-factor authentication and is used solely for support. Opening a family account writes a row into that family’s own activity feed, visible to its parents, with the time and the support ticket it relates to; each read of a family’s records through the operator tools is logged with the time and a stated reason. Individual actions taken inside a support session are not separately logged today.',
      },
      {
        title: '11. How long data is kept',
        body: 'Records expire on a schedule and are deleted automatically: Screen Time and per-app usage, web history, video history, location history and the activity feed after 30 days; SOS alerts, safety Check-Ins and extra-time requests after 90 days; weekly reports after 365 days. Pairing codes expire within minutes, and a browser sign-in lasts 7 days when the app granted it or 8 hours when the Parent PIN did. Some records have no expiry today and are kept until the family account is deleted: the account and its settings, saved places, child and device records, reward tasks, site-unblock requests, the star chart and the weekly Screen Time history, and the list of apps installed on each device. Support requests, replies and any screenshot attached to them are deleted 90 days after the request is resolved. A record that an account deletion was requested and completed — a hash of the email address and the dates, nothing else — is kept for 12 months after completion so that a disputed deletion can be looked into. Limited records may also remain where required for law, fraud prevention, or app-store transactions.',
      },
      {
      ...
      },
      {
        ...
        body: 'Depending on applicable law, users may request access, correction, deletion, restriction, objection, or withdrawal of consent. Such requests, including for a copy of the data held about a family, go to support@kidgate.app; there is no self-service export today. Message monitoring and AI confirmation can be switched off at any time without affecting the rest of the service. Location, notification, camera and device permissions can be disabled in the operating system, but the features that rely on them will stop or become incomplete, and KidGate reports on the parent’s screen when that has happened rather than showing a control that no longer works.',
      },
      {
      ...
      },
      {
        ...
        body: 'This policy may change as features or laws change. Material updates will be communicated in the app or through an appropriate distribution channel. Privacy requests and questions go to support@kidgate.app, the address also published on KidGate’s app-store listings.',
      },
    ],
  },
  termsOfService: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: 'Enforcing a rule requires software on the device it applies to, and each platform grants it differently: Apple’s Screen Time framework on iPhone and iPad, an accessibility service and a device-admin grant on Android, an accessibility service on Android TV, a system extension and a background agent on macOS, a background service on Windows, and a browser extension in Chrome. You are installing this yourself, on a device you are entitled to manage, and you may remove it at any time from that device. Removing it, or withdrawing a permission it depends on, stops enforcement on that device — KidGate will tell you it has happened, but cannot prevent it.',
      },
      {
      ...
      },
      {
        ...
        body: 'Purchases, renewals, cancellations, and refunds are handled under the terms of Apple App Store or Google Play, and a subscription can only be bought or restored from the KidGate app for iPhone, iPad or Android — a family whose devices are all computers, televisions or Chromebooks needs one of those to pay. One subscription covers the whole family and only the family owner can buy or restore it; a purchase attempted from another parent’s account is refused. Monthly and yearly plans renew automatically unless cancelled through the store at least 24 hours before the period ends. The one-time purchase does not renew, covers the number of child devices stated in the app, and lasts for as long as KidGate is offered; section 12 says what happens if that ends. Plan prices and features may change after notice required by law and store rules.',
      },
      {
      ...
      },
      {
        ...
        body: 'You may stop using the service and request account deletion. Deletion holds for 14 days and can be cancelled in that window; after it, the family account and its data are removed permanently. KidGate may suspend or terminate service for violations, safety risks, or legal demands. Parties should first try in good faith to resolve disputes, starting with an email to support@kidgate.app. These terms are governed by the laws of Vietnam and disputes go to the competent courts of Vietnam, except where the mandatory consumer-protection rules of the country you live in give you the right to a different law or court.',
      },
      {
        title: '17. General',
      ...
      },
      {
        ...
        body: 'These terms may be updated. Material changes will be communicated appropriately; continued use after the effective date accepts updated terms where law permits. Questions go to support@kidgate.app.',
      },
    ],
  },
} as const;
