export const webFilter = {
  ...
  mergeNotice:
    'На устройствах {{name}} были разные настройки веб-фильтра. Сохранение применит ко всем один набор: там, где настройки расходились, останется более строгий вариант.',
  ...
  heroTitle: 'Фильтрация нежелательных сайтов',
  heroSubtitleIos:
  ...
  heroSubtitleAndroid:
    'Использует локальный DNS-VPN на Android-устройстве ребёнка, чтобы блокировать известные нежелательные домены в браузерах и многих приложениях.',
  heroSubtitleMacos:
    'Запускает контент-фильтр KidGate на Mac ребёнка, чтобы блокировать известные нежелательные сайты в браузерах и многих приложениях.',
  ...
  safeSearchSectionTitle: 'Безопасный поиск и YouTube',
  safeSearchSectionSubtitle:
    'Принуждает Google, Bing и DuckDuckGo к безопасным результатам и блокирует YouTube в безопасном режиме. Требует включённого веб-фильтра.',
  safeSearchLabel: 'Принудительный безопасный поиск',
  safeSearchHint:
    'Фиксирует Google SafeSearch, безопасный режим YouTube, Bing и DuckDuckGo на строгой настройке. Android, Android TV и Chrome.',
  safeSearchStrictNote:
    'YouTube работает в самом строгом режиме: комментарии скрыты, а часть обычных видео тоже блокируется. Ребёнок не может отключить это в своём аккаунте.',
  ...
  infoLine1Android:
    'KidGate создаёт на устройстве отдельное подключение: оно проверяет, какие сайты запрашиваются, и блокирует те, что попадают в выбранные вами категории.',
  infoLine2Android:
  ...
  infoLine4Android:
    'В Настройках откройте «Сеть и интернет», затем «Личный DNS-сервер» и выберите «Отключено».',
  infoLine1Macos:
  ...
  privateDnsBannerBody:
    'Частный DNS включён, поэтому веб-фильтр можно обойти. Отключите его, чтобы фильтр работал.',
  ...
  heroSubtitleWindows:
    'Запускает собственный резолвер KidGate на компьютере ребёнка, чтобы блокировать известные нежелательные сайты во всех браузерах.',

  toggleHintWindows:
    'На компьютере ничего подтверждать не нужно. Фоновая служба KidGate включает фильтр за несколько секунд.',

  infoLine1Windows:
    'KidGate запускает на компьютере резолвер, который проверяет, какие сайты запрашиваются, и блокирует попадающие в ваши категории.',

  infoLine2Windows:
    'Chrome, Edge и Firefox удерживает на нём настройка, которую применяет KidGate. У ребёнка ничего не спрашивают.',

  infoLine3Windows:
    'Нужна фоновая служба KidGate. Если веб-фильтр остаётся выключенным, переустановите KidGate на компьютере от имени администратора.',

  infoLine4Windows:
    'Фильтр читает только имена сайтов. Он не видит содержимое страницы, а сайт, запрошенный только что, может открываться ещё несколько минут.',

  windowsFilterNote: 'Использует собственный резолвер KidGate в Windows',
  webFilteringNote:
    'iOS использует фильтр контента для взрослых из Экранного времени. Android, Mac и Windows используют собственный блок-лист KidGate.',
  ...
  androidOnlyCategory: 'Недоступно на iPhone — работает на Android и Mac',
  iosCategoryNote:
    'iPhone поддерживает только {{category}} — через собственный фильтр Apple. Остальные категории действуют на устройствах ребёнка — Android и Mac.',
  ...
  privateDnsStep1: 'Откройте Настройки на этом устройстве.',
  privateDnsStep2: 'Выберите «Сеть и интернет».',
  privateDnsStep3: 'Откройте «Личный DNS-сервер» и выберите «Отключено».',
  vpnConsentStepAllow:
    'Выберите «ОК» в запросе VPN от Android. Пока фильтр работает, в строке состояния остаётся значок ключа.',
} as const;
