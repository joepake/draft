export const videoHistory = {
  title: 'Vidéos regardées',
  fallbackDeviceName: 'Appareil de l’enfant',
  toggleLabel: 'Enregistrer les vidéos regardées',
  toggleHint:
    'YouTube dans le navigateur, ainsi que l’appli YouTube sur les téléphones Android et les téléviseurs. Sur TV, les Shorts ne sont pas listés, car l’appli TV ne les identifie pas comme des Shorts.',
  toggleFailed: 'Impossible de mettre à jour l’historique vidéo. Réessayez.',
  unsupportedNote:
    'Cet appareil ne peut pas signaler les vidéos regardées. Fonctionne sur Android et dans Chrome.',

  unsupportedNeedsExtension:
    'KidGate ne peut pas voir sur cet ordinateur quelles vidéos sont regardées : il lit des noms de sites, pas le contenu d’une page. L’extension KidGate pour Chrome le peut : ajoutez-la sur cet ordinateur et elle signalera les vidéos regardées comme un appareil à part.',
  viaExtension: 'Via l’extension',
  extensionGuideTitle: 'Configurez-la dans Chrome',
  extensionStepOpenChrome: 'Ouvrez Chrome sur cet ordinateur',
  extensionStepStore: 'Allez sur le Chrome Web Store',
  extensionStepSearch: 'Recherchez « KidGate »',
  extensionStepInstall: 'Cliquez sur « Ajouter à Chrome »',
  extensionStepConnect:
    'Ouvrez KidGate dans Chrome et connectez-la : elle apparaît comme un appareil à part',
  extensionStepEnable: 'Activez « Enregistrer les vidéos regardées » pour cet appareil',
  emptyTitle: 'Rien de regardé pour l’instant',
  emptyBody:
    'Les vidéos apparaissent ici dès que votre enfant regarde avec KidGate actif.',
  emptyBodyOff: 'Activez l’enregistrement ci-dessus pour lister les vidéos regardées.',
  viewsMeta_one: '{{count}} fois',
  viewsMeta: '{{count}} fois',
  dayToday: 'Aujourd’hui',
  dayYesterday: 'Hier',
  dayCount_one: '{{count}} vidéo',
  dayCount: '{{count}} vidéos',
  heroVideos: 'Vidéos',
  heroChannels: 'Chaînes',
  heroViews: 'Lectures',
  emptyStepEnable: 'Activez l’enregistrement',
  emptyStepWatch: 'Votre enfant regarde YouTube',
  emptyStepAppear: 'Les vidéos s’affichent ici',
  unknownChannel: 'Chaîne inconnue',
  heroTopChannel: 'Le plus regardé',
  readerLayoutChanged:
    'Les Shorts ne sont pas enregistrés sur cet appareil pour le moment : l’application YouTube a changé et KidGate doit être mis à jour. Les autres vidéos sont toujours enregistrées.',
  openAction: 'Ouvrir sur YouTube',
  searchAction: 'Rechercher cette vidéo sur YouTube',
  openFailed: 'Impossible d’ouvrir YouTube.',
};
