...
/**
 * The BCP-47 tag to hand an outside service that does its own localisation.
 *
 * Only `pt` differs from its own code, and it differs for a reason that is not
 * cosmetic: the pack is Brazilian ("Português (Brasil)"), while a bare `pt`
 * means European Portuguese to anything that resolves it — so a service told
 * `pt` answers in the wrong Portuguese rather than falling back to English,
 * which is the failure nobody reports.
 *
 * A service that does not know a tag falls back to its own default, so an
 * unsupported language here costs English, never a broken request.
 */
...
/** `toBcp47('pt')` → `'pt-BR'`. Anything unknown answers `'en'`. */
export function toBcp47(language: string): string {
  return BCP47_BY_LANGUAGE[language as AppLanguage] ?? 'en';
}
