...
export default {
  ...
  home: {
    ...
    only1Text:
      'Android TV では、1 日の上限、利用禁止時間、アプリのブロック、ウェブフィルタが使えます。テレビでのブロックは最善努力までです — ブロック中のアプリはホーム画面に戻されます — また、ソファから SOS や延長申請を送ることはできません。ビルドはすでに実機で動いており、ストアでの公開を待っている段階です。プラットフォーム一覧が「予定」となっているのはそのためです。ほとんどのペアレンタルコントロールはスマホで止まります。',
    ...
    faq1Q: '無料トライアルはありますか？',
    faq1A:
      'はい。7 日間のお試し期間は最初の保護者のデバイスと子どものデバイスを接続した時点で始まり、Premium のすべての機能を含みます。終了後も、設定したルール — 1 日の上限、利用禁止時間、ブロックされたアプリ、ウェブフィルタ、デバイスロック、延長申請、ごほうびタスク — は子どものデバイス 1 台で無料のまま働き続け、そのデバイスに現在地をたずねることもできます。Premium で戻ってくるのは、リアルタイムの利用状況、履歴、週次レポート、位置情報の追跡です。',
    faq2Q: '何台まで管理できますか？',
    faq2A:
  ...
  about: {
    ...
    value4Text:
      'ひとつの Premium サブスクリプションで、すべての保護者とすべてのお子さまのデバイスをカバーします。1 日の上限、利用禁止時間、ブロックされたアプリ、ウェブフィルタはお子さまのデバイス 1 台で無料のまま働き続けるので、安全のためのルールが有料の壁の向こうに置かれることはありません。',
    makeEyebrow: '作っているもの',
