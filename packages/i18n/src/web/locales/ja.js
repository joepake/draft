...
export default {
  ...
  home: {
    ...
    feature4Text:
      'アダルトやギャンブルのサイトを拒否したうえで、そのデバイスが実際に見に行ったサイトと、止められたサイトを確認できます。',
    ...
    platformsSub:
      'スマホでもパソコンでも、同じルールと同じ家族アカウント。パソコン版はストアではなく、このサイトからインストールします。',
    ...
    faq1Q: '無料トライアルはありますか？',
    faq1A:
      'あります。最初の保護者のデバイスと子どものデバイスが接続された時点で始まり、すべての Premium 機能が使えます。 期間が終わっても、1日の上限・休止時間・位置情報は子どもの端末 1 台で無料のまま使えます。',
    faq2Q: '何台まで管理できますか？',
    faq2A:
      '1 つのサブスクで家族全体をカバーします。同じプランで複数の子どものデバイスと複数の保護者を管理できます。',
    faq3Q: '子どもが KidGate を削除したり回避したりできますか？',
    faq3A:
      '重要な設定は保護者 PIN で守られ、子どものデバイスで主要な権限がオフになると保護アラートですぐに知らせます。',
    faq4Q: 'すべてをパソコンから管理できますか？',
    faq4A:
      'ウェブダッシュボードにログインしてレポートを読めます。上限の変更やデバイスのロックはスマホから承認するため、パスワードが漏れただけでは操作できません。',
  ...
  login: {
    ...
    qrWhy:
      'コントロールのロックを解除できるのはスマホからの承認だけです。デバイスのロックや上限の変更はアプリ側に残ります。下の方法は、レポート閲覧のためのログインです。',
  ...
  dash: {
    ...
    viewOnlyTitle: '閲覧のみ。',
    viewOnlyBody:
      'デバイスのロック、上限の変更、リクエストの承認を行うには、いったんログアウトし、KidGate アプリで QR コードを読み取って再ログインしてください。接続済みの保護者スマホからの承認だけがコントロールのロックを解除します。チェックインはどちらの場合でもここから使えます。',
  ...
  support: {
    ...
    faq4A:
      '最初の保護者のデバイスと子どものデバイスが接続された時点で始まり、すべての機能を利用できます。子どものデバイスを削除してもトライアルはリセットされません。期間が終わったら、Premium に登録すると KidGate を使い続けられます。',
  ...
  download: {
    ...
    button: 'ダウンロード',
    warningSub:
      'いずれのOSも、自社ストア以外から、まだ確認済みリストに載っていない開発者のアプリをインストールする場合には必ずこの警告を出します。KidGate に何か見つかったという意味ではありません。最初の起動を許可する手順は上の各カードに書いてあります。ダウンロードは kidgate.app からのみ行ってください。',
    macosSteps:
      'アプリを一度開き、拒否されるままにします。次にシステム設定、プライバシーとセキュリティを開き、下までスクロールして「このまま開く」を選びます。',
    windowsSteps:
  ...
  about: {
    ...
    storyP3:
      'iPhone、Android、Mac、Windows で動き、どのブラウザからでも開けるダッシュボードがあります。ひとつの家族、ひとつのプラン、すべての端末に。',
    ...
    make5Text:
      'リビングの画面を、お子さま個人の端末ではなく家族で共有する端末として扱います。上限も時間割もスマホと同じです。',
    make6Title: '保護者ダッシュボード',
    make6Text:
