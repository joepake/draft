...
export default {
  ...
  common: {
    ...
    loading: '読み込み中…',
    signOut: 'ログアウト',
    crashTitle: 'このページは動作を停止しました',
    crashBody:
      '再読み込みでたいてい解決します。ご家族の設定とお子さまの端末は変わっていません。',
  ...
  home: {
    ...
    trust3Title: 'スマホとパソコン',
    trust3Text: 'iPhone、Android、Mac、Windows をひとつの家族アカウントで',
    ...
    feature1Text:
      '学校や就寝時間に合わせて、1 日の上限と休止時間を設定できます。時間切れになるとデバイスは自動でロックされます。',
    ...
    feature4Text:
      'スマホでもパソコンでも、アダルトやギャンブルのサイトを拒否します。Premium では、実際に見に行かれたサイトと、止められたサイトを確認できます。',
    ...
    feature7Text:
      '重要な権限がオフになった瞬間に分かります。Premium では、Android の新しいアプリは、開く前に保護者の承認を待ちます。',
    ...
    feature12Text:
      '起きたことが時系列で並びます。デバイスのロック解除、フィルタしたサイト、終えたタスク、届いた通知。今日の分は無料で、Premium では 30 日分が残ります。',
    featurePremium: 'Premium',
    platformsTitle: 'どの画面でも、KidGate はひとつ',
    platformsSub:
      'スマホでもパソコンでも、同じルールと同じ家族アカウント。パソコン版はストアではなく、このサイトからインストールします。Chrome 版と Android TV 版は、ストアの審査を待っています。',
    ...
    why1Text:
      '1 つの Premium サブスクで保護者と子どものすべてのデバイスをカバーし、支払うのは家族のオーナーだけです。無料プランでは、子どものデバイス 1 台が監視の対象として残ります。',
    ...
    only1Text:
      'Android TV では、1 日の上限、休止時間、アプリのブロック、Webフィルターが使えます。テレビでのブロックは最善努力までです — ブロック中のアプリはホーム画面に戻されます — また、ソファから SOS や延長申請を送ることはできません。ビルドはすでに実機で動いており、ストアでの公開を待っている段階です。ほとんどのペアレンタルコントロールはスマホで止まります。',
    ...
    only4Title: 'お子さまの逃げ道',
    only4Text:
      'SOS を 5 秒長押しすると、位置情報とともにすぐ保護者に届きます。Android と Mac では、デバイスのロックも少しのあいだ解除されます。ロック画面から助けを呼べるお子さまに、アプリと争う理由はありません。',
    only5Title: 'ネットがなくても守られるルール',
    only5Text:
      '休止時間と 1 日の上限はデバイス自身が守るため、ルーターを抜いても何も変わりません。テレビは接続がまったくなくても保護者 PIN を受け付けます。',
    ...
    faq1Q: '無料トライアルはありますか？',
    faq1A:
      'はい。7 日間のお試し期間は最初の保護者のデバイスと子どものデバイスを接続した時点で始まり、Premium のすべての機能を含みます。終了後も、設定したルール — 1 日の上限、休止時間、ブロックされたアプリ、Webフィルター、デバイスロック、延長申請、ごほうびタスク — は子どものデバイス 1 台で無料のまま働き続け、そのデバイスに現在地をたずねることもできます。Premium で戻ってくるのは、リアルタイムの利用状況、履歴、週次レポート、位置情報の追跡です。',
    faq2Q: '何台まで管理できますか？',
    faq2A:
    ...
    faq5A:
      'Premium は米国で月額 $6.99 または年額 $39.99 です。App Store または Google Play を通じて課金され、そこではお住まいの地域の通貨で表示されます。買い切りの Lifetime プランは、子どものデバイス 3 台までをカバーします。無料プランに期限はありません。',
  ...
  authError: {
    ...
    rateLimited:
      'このネットワークからのサインインコードが多すぎます。{{minutes}}分後にもう一度お試しください。',
  ...
  live: {
    ...
    noAccessTitle: 'このアカウントに家族がありません',
    noAccess:
  ...
  dash: {
    ...
    refresh: '更新',
    liveOnApp: 'リアルタイムの更新はアプリで',
    pinTitle: '保護者用 PIN を入力',
    pinBody:
      'アプリで使うのと同じ 6 桁です。このブラウザは 8 時間解除されたままになります。アプリから承認すると、7 日間サインインしたままになります。',
    pinLabel: '保護者用 PIN',
    pinSubmit: 'ロック解除',
    ...
    noDeviceBody:
      'スマホで KidGate を開き、*家族* に進んでスキャンアイコン（*コードをスキャン*）をタップし、子どものデバイスに表示される QR コードを読み取ってください。接続から数秒でここに表示されます。',
    ...
    webFilterCatsTitle: 'Webフィルターのカテゴリ',
  ...
  report: {
    ...
    emptyBody: 'レポートは毎週月曜の朝に届き、その前の 7 日間が対象です。',
    noUsage:
  ...
  support: {
    ...
    contactResponse: '**返信の目安:** 通常 1 営業日以内',
    contactNote:
    ...
    start3:
      '**3. 子どものデバイスを接続する。** 子どものデバイスに KidGate をインストールし、*これは子どものデバイスです* を選びます。保護者のデバイスで *家族* を開いてスキャンアイコン（*コードをスキャン*）をタップし、子どものデバイスに表示された QR コードを読み取ります（または 6 文字のコードを入力します）。子どものデバイスで接続を確認します。',
    start4:
    ...
    start5:
      '**5. コントロールを設定する。** 保護者のデバイスで子どものデバイスカードを開き、1 日の上限、休止時間、ブロックするアプリ、Webフィルター、位置情報の各機能を設定します。',
    startNote:
    ...
    faq2A:
      '子どものデバイスで KidGate を開き、*これは子どものデバイスです* を選ぶと、QR コードと 6 文字のコードが表示されます。保護者のデバイスで *家族* を開いてスキャンアイコン（*コードをスキャン*）をタップし、QR コードを読み取る（推奨）か、コードを手入力します。その後、子どものデバイスで保護者の名前を確認します。コードには有効期限があります。接続に失敗したら、子どものデバイスで *新しいコード* をタップしてやり直してください。',

    faq3Q: '保護者 2 人で同じ家族を管理できますか？',
    faq3A:
      'できます。家族のオーナーのデバイスで *家族 → + → 保護者を招待* を開き、招待用の QR コードまたはコードを共有します。もう一人の保護者は KidGate をインストールし、保護者としてログインして *家族* を開き、スキャンアイコン（*コードをスキャン*）をタップして、QR コードを読み取るか招待コードを入力します。その後オーナーがリクエストを承認します。1 つのサブスクで家族全体をカバーし、支払うのはオーナーだけです。',

    faq4Q: '無料トライアルはどのように動きますか？',
    faq4A:
      '7 日間のお試し期間は最初の保護者のデバイスと子どものデバイスを接続した時点で始まり、すべての機能を使えます。子どものデバイスを削除してもリセットされません。終了後も、ルールはすべて子どものデバイス 1 台で無料のまま働きます。Premium では、リアルタイムの利用状況、履歴、週次レポート、すべてのデバイスが使えます。',
    ...
    faq8Q: 'ロックや休止時間が効かないのはなぜですか？',
    faq8A:
      'Android では、ロックに *他のアプリの上に表示* と *ユーザー補助* のヘルパーの有効化、さらにバッテリー無制限が必要です。Xiaomi、Samsung、Oppo、Vivo などのデバイスでは、自動起動も許可し、「スリープ中のアプリ」の一覧から KidGate を外してください（子どものデバイスの *ステータス → 自動起動を許可* を参照）。iOS ではロックはスクリーンタイムの許可に依存します。あとから権限がオフになった場合は、保護者のデバイスに保護アラートが届きます。',
  ...
  download: {
    ...
    macosSteps:
      'ダウンロードしたパッケージを開き、インストーラの案内に従います。そのあと macOS が一度だけ、「ログイン項目と機能拡張」で KidGate のシステム機能拡張を許可するよう求めます。許可するまでWebフィルターは動きません。',
    windowsSteps:
  ...
  about: {
    ...
    storyP2:
      'そこで、自分たちの家庭で使いたい形を作りました。保護者が 1 日の上限、休止時間、アプリのブロック、Webフィルターを一度設定すれば、端末はそのとおりに動きます。お子さまは保護者と同じ数字を見て、延長を申請でき、デバイスがオンラインであればいつでも SOS で保護者に連絡できます。KidGate は、そこにいないふりをしません。',
    storyP3:
    ...
    valuesSub:
      '最初の機能より先に決めたもので、それ以降のすべての機能をこれに照らして確かめています。',
    ...
    value2Text:
      '広告は一切ありません。お子さまに関する情報を広告に使うことも、第三者に販売することもありません。ご家族のアカウントとその中身は、アプリからでもこのサイトからでも、いつでも削除を申請でき、14 日後に消去されます。',
    ...
    value4Text:
      'ひとつの Premium サブスクリプションで、すべての保護者とすべてのお子さまのデバイスをカバーします。1 日の上限、休止時間、ブロックされたアプリ、Webフィルターはお子さまのデバイス 1 台で無料のまま働き続けるので、安全のためのルールが有料の壁の向こうに置かれることはありません。',
    makeEyebrow: '作っているもの',
    makeTitle: '画面のある場所すべてに、ひとつの KidGate',
    makeSub: '同じルールを一度だけ書き、各プラットフォームが許す範囲で実行します。',
    make1Title: 'iPhone と iPad',
    make1Text:
      '1 日の上限、休止時間、アプリのブロックを、Apple 自身のスクリーンタイムの仕組みで実現します。',
    make2Title: 'Android',
    make2Text:
      '時間の上限、アプリのブロック、全画面ロック、Webフィルターに加え、新しいアプリが増えたときの通知。',
    make3Title: 'macOS',
    make3Text:
    ...
    make6Title: 'Chrome',
    make6Text:
      'KidGate がすでに入っているパソコンでも、入れられないパソコンでも、Chrome の中で同じWebフィルターを働かせるブラウザ拡張機能です。完成して接続も済んでおり、Chrome Web Store の審査を待っています。',
