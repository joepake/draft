...
export function mergeDeviceWebFilters(
  ...
  // Stricter reading, like the two switches above: on anywhere is on.
  const safeSearch = present.some(controls => controls.safeSearchEnabled === true);
  ...
  const diverged =
    ...
    present.some(controls => (controls.safeSearchEnabled === true) !== safeSearch) ||
