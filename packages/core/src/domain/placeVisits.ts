...
export function placeForFix(
  /*
   * `DeviceLocation`, not `LocationHistoryEntry`: this reads latitude,
   * longitude and accuracy and never the row's `id`. Widened so the *current*
   * fix on a device document can be asked the same question a trail row can —
   * `apps/dashboard` names its device row from `Device.lastLocation`, which
   * has no id, and narrowing it to a history row is what left that surface
   * with coordinates while the phone said "Home".
   */
