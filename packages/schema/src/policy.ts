...
import type { AppRef, AppRefId, Millis, Minutes } from './primitives';
...
export interface WebFilterPolicy {
  ...
  /**
   * Force the search engines' safe modes (`ContentFilterRules.safeSearch`
   * carries the host table). Optional because policies built before the
   * field existed are still valid; absent is off.
   */
  safeSearch?: boolean;
}

/**
 * The app install quarantine, as the device enforces it.
 *
 * A package whose OS install time is after `sinceMs` and whose id is not in
 * `approved` may not be opened. Present on `AppPolicy` only while the parent's
 * switch is on — absent is "no quarantine", never "quarantine everything".
 * `DeviceControls.appInstallApprovalEnabled` explains the shape.
 */
export interface AppInstallApprovalPolicy {
  /** Epoch ms; installs at or before this instant are not quarantined. */
  sinceMs: Millis;
  /** Package ids a parent has let through. */
  approved: AppRefId[];
}
...
export interface AppPolicy {
  ...
  /** Absent when the parent's install-approval switch is off. */
  installApproval?: AppInstallApprovalPolicy;
}
